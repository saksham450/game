...
class MainMenu:
    ...
    def draw(self):
        ...
        title = self.title_font.render("Pictonary!", True, (0, 0, 0))
        self.win.blit(title, (self.WIDTH / 2 - title.get_width() / 2, 50))

        name = self.name_font.render("Type a Name: " + self.name, True, (0, 0, 0))
        self.win.blit(name, (100, 400))

        if self.waiting:
            enter = self.enter_font.render("In Queue...", True, (0, 0, 0))
            self.win.blit(enter, (self.WIDTH / 2 - title.get_width() / 2, 800))
        else:
            enter = self.enter_font.render("Press enter to join a game...", True, (0, 0, 0))
